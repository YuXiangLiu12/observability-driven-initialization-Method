...
void pubOdometry(const Estimator &estimator, const std_msgs::Header &header)
{
    if (estimator.solver_flag == Estimator::SolverFlag::NON_LINEAR)
    {
        ...
        // write result to file
        Quaterniond tmp_Q_pre;
        tmp_Q_pre = Quaterniond(estimator.Rs_pre);
        ofstream foutC_pre(VINS_RESULT_PATH_pre, ios::app);
        foutC_pre.setf(ios::fixed, ios::floatfield);
        foutC_pre.precision(0);
        foutC_pre << header.stamp.toSec() * 1e9 << ",";
        foutC_pre.precision(5);
        foutC_pre << estimator.Ps_pre.x() << ","
                    << estimator.Ps_pre.y() << ","
                    << estimator.Ps_pre.z() << ","
                    << tmp_Q_pre.w() << ","
                    << tmp_Q_pre.x() << ","
