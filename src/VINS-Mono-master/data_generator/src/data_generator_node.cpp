...
// 模拟 IMU 相对相机的固定时间延迟(秒)，即 VINS 中的 td 参数
// IMU 在真实时刻 t 的测量被标记为 t + IMU_TIME_DELAY
#define IMU_TIME_DELAY 0.030
...
int main(int argc, char **argv)
{
    ...
    // Open ground truth file (overwritten on every run)
    std::string gt_path = ros::package::getPath("data_generator") + "/ground_truth.txt";
    std::ofstream gt_file(gt_path);
    if (!gt_file.is_open())
    {
        ROS_ERROR("Failed to open ground truth file: %s", gt_path.c_str());
        return -1;
    }
    gt_file << "# time x y z qx qy qz qw vx vy vz ba_x ba_y ba_z bg_x bg_y bg_z" << std::endl;
    ...
    while (ros::ok())
    {
        ...
        // Write ground truth (world frame): pose, velocity, bias
        Vector3d world_velocity = rotation * velocity; // generator velocity is in body frame
        Vector3d ba = generator.getAccBias();
        Vector3d bg = generator.getGyrBias();
        gt_file << std::fixed << std::setprecision(9)
                << current_time << " "
                << position(0) << " " << position(1) << " " << position(2) << " "
                << q.x() << " " << q.y() << " " << q.z() << " " << q.w() << " "
                << world_velocity(0) << " " << world_velocity(1) << " " << world_velocity(2) << " "
                << ba(0) << " " << ba(1) << " " << ba(2) << " "
                << bg(0) << " " << bg(1) << " " << bg(2) << std::endl;
        ...
        imu.header.stamp = ros::Time(current_time + IMU_TIME_DELAY);  // 模拟 IMU 时间戳延迟
        ...
        if (publish_count % generator.IMU_PER_IMG == 0)
        {
            ...
            sensor_msgs::ChannelFloat32 u, v, vel_x, vel_y;
            ...
            for (auto &id_pts : generator.getImage())
            {
                ...
                feature.points.push_back(p); // normalized camera coordinates (z = 1)
                ids.values.push_back(id);
                // pixel coordinates, consistent with the rendering mapping below
                u.values.push_back((p.x + 1) * 0.5 * 600);
                v.values.push_back((p.y + 1) * 0.5 * 600);
                vel_x.values.push_back(0.0);
                vel_y.values.push_back(0.0);
            ...
            feature.channels.push_back(ids);     // feature id (id * NUM_OF_CAM + camera_id)
            feature.channels.push_back(u);       // pixel u
            feature.channels.push_back(v);       // pixel v
            feature.channels.push_back(vel_x);   // pixel velocity x (set to 0)
            feature.channels.push_back(vel_y);   // pixel velocity y (set to 0)
    ...
    gt_file.close();
    ROS_INFO("Ground truth saved to %s (%lu lines)", gt_path.c_str(), (unsigned long)(publish_count + 1));
    return 0;
}
