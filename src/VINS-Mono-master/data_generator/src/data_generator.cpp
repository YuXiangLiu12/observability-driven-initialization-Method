...
// 姿态激励参数：增大幅值 + 缩短周期，保证相邻帧间旋转明显（Stage 2 外参估计需要）
// 原: Rx(30°)Ry(40°) 周期=MAX_TIME=40s -> 峰值角速度仅 ~6°/s，每帧(50ms)扫过 <0.3°
// 改: 三轴摆动 + 周期 8s -> 峰值角速度 ~80°/s，每帧扫过 ~4°(>> 特征噪声，SNR 高)
#define ROT_AMP_X 60     // 绕 X 轴摆动幅值(度)
#define ROT_AMP_Y 70     // 绕 Y 轴摆动幅值(度)
#define ROT_AMP_Z 30     // 绕 Z 轴摆动幅值(度)，三轴激励
#define ROT_PERIOD 8.0   // 姿态摆动周期(秒)，独立于 MAX_TIME(轨迹周期)

// 0.001 0.002 0.003 m/s^2
#define BIAS_ACC 1

// 0.002 0.003 0.004 rad/s
#define BIAS_GYR 1

// Ric[0] << 0, 0, -1,
//          -1, 0,  0,
//           0, 1,  0;
// Tic[0] << 0.0, 0.2, 0.6;   // 相机在 IMU 前方 x=0, y=0.2, z=0.6
...
DataGenerator::DataGenerator()
{
    ...
    // 噪声量级（每样本 std）：
    //   陀螺 0.05 rad/s：预积分累积旋转误差 ~0.05*0.005*sqrt(10) ≈ 2.5e-3 rad ≈ 0.14°/帧
    //   加计 0.05 m/s²：对旋转优化器无影响，仅用于完整 pipeline 测试
    //   图像 0.3px（90°FOV/460px 约定）：视线角噪声 ~0.075°
    acc_cov = 0.05 * 0.05 * Matrix3d::Identity();
    gyr_cov = 0.05 * 0.05 * Matrix3d::Identity();
...
Matrix3d DataGenerator::getRotation()
{
    ...
    // 三轴独立正弦摆动，周期 ROT_PERIOD(与轨迹周期 MAX_TIME 解耦)，保证姿态变化明显
    const double w = 2.0 * M_PI / ROT_PERIOD;
    return (AngleAxisd(ROT_AMP_X / 180.0 * M_PI * sin(w * t), Vector3d::UnitX()) *
            AngleAxisd(ROT_AMP_Y / 180.0 * M_PI * sin(w * t), Vector3d::UnitY()) *
            AngleAxisd(ROT_AMP_Z / 180.0 * M_PI * sin(w * t), Vector3d::UnitZ())).toRotationMatrix();
}
...
Vector3d DataGenerator::getAngularVelocity()
{
    ...
    if(IMU_NOISE)
    {
        // 测量模型：ω_meas = ω_true + bg + 噪声（bias 必须保留，与 BIAS_GYR=1 分支一致）
...
#if BIAS_GYR
        return disturb + Vector3d(skew(2, 1) + 0.002, -skew(2, 0) + 0.003, skew(1, 0) + 0.004);
#else
    ...
    {
#if BIAS_GYR
        return Vector3d(skew(2, 1) + 0.002, -skew(2, 0) + 0.003, skew(1, 0) + 0.004);
#endif
...
Vector3d DataGenerator::getLinearAcceleration()
{
    ...
    if(IMU_NOISE)
    {
        // 测量模型：a_meas = R^T*(a_world + g) + ba + 噪声（bias 必须保留，与 BIAS_ACC=1 分支一致）
...
#if BIAS_ACC
        return getRotation().inverse() * Vector3d(ddx, ddy, ddz + 9.805) + disturb + Vector3d(0.001, 0.002, 0.003);
#else
        return getRotation().inverse() * Vector3d(ddx, ddy, ddz + 9.805) + disturb;
#endif
    }
    else
    {
#if BIAS_ACC
        return getRotation().inverse() * Vector3d(ddx, ddy, ddz + 9.805) + Vector3d(0.001, 0.002, 0.003);
#endif
...
Vector3d DataGenerator::getAccBias()
{
    Vector3d bias = Vector3d::Zero();
#if BIAS_ACC
    bias = Vector3d(0.001, 0.002, 0.003);
#endif
    return bias;
}

Vector3d DataGenerator::getGyrBias()
{
    Vector3d bias = Vector3d::Zero();
#if BIAS_GYR
    bias = Vector3d(0.002, 0.003, 0.004);
#endif
    return bias;
}
